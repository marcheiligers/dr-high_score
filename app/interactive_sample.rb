  ...
  args.state.widgets ||= [
    ...
    args.state.key_input = Input::Text.new(
      ...
      on_unhandled_key: lambda do |key, input|
        ...
        when :tab
          input.blur
          args.state.encrypted_input.focus
          args.state.encrypted_input.select_all
        when :enter
    ...
    args.state.encrypted_input = Input::Text.new(
      x: 20,
      y: 180,
      w: 480,
      text_color: 0xFFFFFF,
...
def encrypt_and_use(args)
  token = args.state.key_input.value.to_s
  if token.length != PURPLE_TOKEN_KEY.length
    args.gtk.notify! "Enter a valid token"
  else
    encrypted_token = HighScore::BadCrypto.encrypt(token)
    args.state.purple_token = HighScore::PurpleToken.new(encrypted_token)
    args.state.encrypted_input.value = encrypted_token
  end
end
...
def tick(args)
  ...
  args.outputs.labels << [
    ...
    { x: 20, y: 380, text: 'Use your own key', r: 255, g: 255, b: 255, size_px: 28 },
